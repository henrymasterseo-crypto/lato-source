...
class LatoFontMake:
    ...
    def build_fontmake(
        ...
        print(
            project.run_from_designspace(
                designspace_path=designspace_path, **fontmake_args
            )
        )
    ...
    def buildVF2(self, basefont):
        ...
        designspace_path = os.path.join(
            self.cwd, "..", "sources", basefont + "2M.designspace"
        )
        output = ["variable"]
        ...
        interpolate = False
        self.build_fontmake(
            designspace_path, output, output_dir, remove_overlaps, interpolate
        )

    def buildVF3(self, basefont):
        print("\n\nBuilding %s 3-master TTF VF..." % basefont)
        designspace_path = os.path.join(
            self.cwd, "..", "sources", basefont + "3M.designspace"
        )
        output = ["variable"]
    ...
    def buildTest(self, basefont):
        ...
        designspace_path = os.path.join(
            self.cwd, "..", "sources", basefont + "Test.designspace"
        )
        output = ["ttf"]
